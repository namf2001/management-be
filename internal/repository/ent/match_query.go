...
func (mq *MatchQuery) QueryMatchPlayers() *MatchPlayerQuery {
...
func (mq *MatchQuery) QueryTeam() *TeamQuery {
...
func (mq *MatchQuery) Clone() *MatchQuery {
	...
	return &MatchQuery{
		...
		withMatchPlayers: mq.withMatchPlayers.Clone(),
...
func (mq *MatchQuery) WithMatchPlayers(opts ...func(*MatchPlayerQuery)) *MatchQuery {
	query := (&MatchPlayerClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withMatchPlayers = query
	return mq
}

// WithTeam tells the query-builder to eager-load the nodes that are connected to
// the "team" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MatchQuery) WithTeam(opts ...func(*TeamQuery)) *MatchQuery {
	query := (&TeamClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Match.Query().
//		GroupBy(match.FieldOpponentTeamID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Match.Query().
//		Select(match.FieldOpponentTeamID).
//		Scan(ctx, &v)
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	if query := mq.withMatchPlayers; query != nil {
		if err := mq.loadMatchPlayers(ctx, query, nodes,
			func(n *Match) { n.Edges.MatchPlayers = []*MatchPlayer{} },
			func(n *Match, e *MatchPlayer) { n.Edges.MatchPlayers = append(n.Edges.MatchPlayers, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withTeam; query != nil {
		if err := mq.loadTeam(ctx, query, nodes, nil,
			func(n *Match, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MatchQuery) loadMatchPlayers(ctx context.Context, query *MatchPlayerQuery, nodes []*Match, init func(*Match), assign func(*Match, *MatchPlayer)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(matchplayer.FieldMatchID)
	}
	query.Where(predicate.MatchPlayer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(match.MatchPlayersColumn), fks...))
	}))
...
func (mq *MatchQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Match, init func(*Match), assign func(*Match, *Team)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "opponent_team_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MatchQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := mq.ctx.Fields; len(fields) > 0 {
		...
		if mq.withTeam != nil {
			_spec.Node.AddColumnOnce(match.FieldOpponentTeamID)
		}
	}
