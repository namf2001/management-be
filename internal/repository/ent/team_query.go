...
func (tq *TeamQuery) QueryMatches() *MatchQuery {
...
func (tq *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withMatches: tq.withMatches.Clone(),
...
func (tq *TeamQuery) WithMatches(opts ...func(*MatchQuery)) *TeamQuery {
	query := (&MatchClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withMatches = query
	return tq
}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withMatches; query != nil {
		if err := tq.loadMatches(ctx, query, nodes,
			func(n *Team) { n.Edges.Matches = []*Match{} },
			func(n *Team, e *Match) { n.Edges.Matches = append(n.Edges.Matches, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeamQuery) loadMatches(ctx context.Context, query *MatchQuery, nodes []*Team, init func(*Team), assign func(*Team, *Match)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(match.FieldOpponentTeamID)
	}
	query.Where(predicate.Match(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.MatchesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "opponent_team_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
