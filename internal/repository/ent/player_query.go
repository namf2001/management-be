...
type PlayerQuery struct {
	...
	withPlayerStatistic *PlayerStatisticQuery
...
func (pq *PlayerQuery) QueryMatchPlayers() *MatchPlayerQuery {
...
// QueryPlayerStatistic chains the current query on the "player_statistic" edge.
func (pq *PlayerQuery) QueryPlayerStatistic() *PlayerStatisticQuery {
	query := (&PlayerStatisticClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PlayerQuery) QueryDepartment() *DepartmentQuery {
...
func (pq *PlayerQuery) Clone() *PlayerQuery {
	...
	return &PlayerQuery{
		...
		withMatchPlayers:    pq.withMatchPlayers.Clone(),
		withPlayerStatistic: pq.withPlayerStatistic.Clone(),
...
func (pq *PlayerQuery) WithMatchPlayers(opts ...func(*MatchPlayerQuery)) *PlayerQuery {
	query := (&MatchPlayerClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withMatchPlayers = query
	return pq
}

// WithPlayerStatistic tells the query-builder to eager-load the nodes that are connected to
// the "player_statistic" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PlayerQuery) WithPlayerStatistic(opts ...func(*PlayerStatisticQuery)) *PlayerQuery {
	query := (&PlayerStatisticClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPlayerStatistic = query
	return pq
}
...
func (pq *PlayerQuery) WithDepartment(opts ...func(*DepartmentQuery)) *PlayerQuery {
	query := (&DepartmentClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Player.Query().
//		GroupBy(player.FieldDepartmentID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Player.Query().
//		Select(player.FieldDepartmentID).
//		Scan(ctx, &v)
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [3]bool{
			pq.withMatchPlayers != nil,
			pq.withPlayerStatistic != nil,
	...
	if query := pq.withMatchPlayers; query != nil {
		if err := pq.loadMatchPlayers(ctx, query, nodes,
			func(n *Player) { n.Edges.MatchPlayers = []*MatchPlayer{} },
			func(n *Player, e *MatchPlayer) { n.Edges.MatchPlayers = append(n.Edges.MatchPlayers, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withPlayerStatistic; query != nil {
		if err := pq.loadPlayerStatistic(ctx, query, nodes, nil,
			func(n *Player, e *PlayerStatistic) { n.Edges.PlayerStatistic = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withDepartment; query != nil {
		if err := pq.loadDepartment(ctx, query, nodes, nil,
			func(n *Player, e *Department) { n.Edges.Department = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadMatchPlayers(ctx context.Context, query *MatchPlayerQuery, nodes []*Player, init func(*Player), assign func(*Player, *MatchPlayer)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(matchplayer.FieldPlayerID)
	}
	query.Where(predicate.MatchPlayer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.MatchPlayersColumn), fks...))
	}))
...
func (pq *PlayerQuery) loadPlayerStatistic(ctx context.Context, query *PlayerStatisticQuery, nodes []*Player, init func(*Player), assign func(*Player, *PlayerStatistic)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(playerstatistic.FieldPlayerID)
	}
	query.Where(predicate.PlayerStatistic(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(player.PlayerStatisticColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *PlayerQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withDepartment != nil {
			_spec.Node.AddColumnOnce(player.FieldDepartmentID)
		}
	}
